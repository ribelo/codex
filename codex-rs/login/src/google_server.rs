...
) -> std::io::Result<(String, Option<String>, &'static str)> {
    match provider_kind {
        GoogleProviderKind::Gemini => Ok((
            gemini_client_id()?,
            gemini_client_secret(),
            GEMINI_TOKEN_URL,
        )),
        GoogleProviderKind::Antigravity => Ok((
                    ...
                    let user_info = match fetch_user_info(&tokens.access_token) {
                        ...
                        Err(err) => {
                            return HandledRequest::Response(
                                Response::from_string(format!("Failed to fetch user info: {err}"))
                                    .with_status_code(500),
                            );
                        }
                    };

                    let google_tokens = GeminiTokenData {
...
fn exchange_code_for_tokens(
    ...
    let expected_state = Url::parse(auth_url)?
        .query_pairs()
        .find_map(|(key, value)| {
            if key == "state" {
...
fn fetch_user_info(access_token: &str) -> Result<GeminiUserInfo, Box<dyn std::error::Error>> {
    ...
    if !response.status().is_success() {
        return Err(format!("User info request failed: {}", response.status()).into());
    }

    let data: JsonValue = response.json()?;
    let email = data["email"].as_str().map(str::to_string);
...
fn persist_google_tokens(
    ...
    google_tokens: GeminiTokenData,
    provider_kind: GoogleProviderKind,
) -> std::io::Result<AuthDotJson> {
    let mut auth = match load_auth_dot_json(codex_home, creds_mode) {
        Ok(Some(auth)) => auth,
        Ok(None) | Err(_) => AuthDotJson::empty(),
    };

    let accounts = provider_accounts_mut(&mut auth, provider_kind);
    accounts.retain(|existing| match (&google_tokens.email, &existing.email) {
        (Some(new_email), Some(existing_email)) => existing_email != new_email,
        _ => true,
    });
    accounts.insert(0, google_tokens);
...
fn build_authorize_url(
    ...
    let mut url = match Url::parse(auth_url) {
        Ok(url) => url,
        Err(_) => return Ok(auth_url.to_string()),
    };
...
mod tests {
    ...
    #[test]
    fn antigravity_tokens_are_stored_separately() -> std::io::Result<()> {
        let dir = tempdir()?;
        ...
        assert_eq!(auth.antigravity_accounts, vec![tokens]);
        assert!(auth.gemini_accounts.is_empty());
        Ok(())
    }

    #[test]
    fn malformed_existing_auth_json_is_ignored_when_persisting_google_tokens() -> std::io::Result<()>
    {
        let dir = tempdir()?;
        std::fs::write(
        ...
        let tokens = sample_tokens("gemini");
        let auth = persist_google_tokens(
            dir.path(),
            AuthCredentialsStoreMode::File,
            tokens.clone(),
