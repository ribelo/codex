...
impl ModelsManager {
    ...
    #[instrument(skip_all, fields(provider = %state.provider.name))]
    ...
    #[cfg(test)]
    fn default_cache_manager_mut(&mut self) -> &mut ModelsCacheManager {
        &mut self
            .provider_states
            .get_mut(&self.default_provider_id)
            .expect("default provider state")
            .cache_manager
    }
    ...
    #[cfg(test)]
    fn build_available_models_for_default_provider(
        &self,
        remote_models: Vec<ModelInfo>,
    ) -> Vec<ModelPreset> {
        self.build_available_models(&self.default_provider_id, remote_models)
    }
}
