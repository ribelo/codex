    ...
    pub fn for_session(
        ...
        if cfg!(unix)
            ...
            && let (Ok(shell_zsh_path), Ok(main_execve_wrapper_exe)) = (
                AbsolutePathBuf::try_from(shell_zsh_path.as_path()).inspect_err(|e| {
                    tracing::warn!("Failed to convert shell_zsh_path `{shell_zsh_path:?}`: {e:?}")
                }),
                AbsolutePathBuf::try_from(main_execve_wrapper_exe.as_path()).inspect_err(|e| {
                    tracing::warn!(
                        "Failed to convert main_execve_wrapper_exe `{main_execve_wrapper_exe:?}`: {e:?}"
                    )
                }),
            )
        {
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let request_permission_enabled = features.enabled(Feature::ExecPermissionApprovals);
...
- The key is to find opportunities to spawn multiple independent subtasks in parallel within the same round, while ensuring each subtask is well-defined, self-contained, and materially advances the main task."#,
            available_models_description = spawn_agent_models_description(&config.available_models),
        ),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    if config.collab_tools {
        ...
        let send_input_handler = Arc::new(SendInputHandler);
        let resume_agent_handler = Arc::new(ResumeAgentHandler);
        let wait_agent_handler = Arc::new(WaitAgentHandler);
        let close_agent_handler = Arc::new(CloseAgentHandler);
        push_tool_spec(
            &mut builder,
        ...
        builder.register_handler("send_input", send_input_handler);
        builder.register_handler("resume_agent", resume_agent_handler);
        builder.register_handler("wait_agent", wait_agent_handler);
        builder.register_handler("close_agent", close_agent_handler);
    }
