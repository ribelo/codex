...
async fn shell_output_stays_json_without_freeform_apply_patch(
    ...
    let responses = shell_responses(
        call_id,
        vec![test_binary_path("echo"), "shell json".to_string()],
        output_type,
    )?;
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let responses = shell_responses(
        call_id,
        vec![test_binary_path("echo"), "freeform shell".to_string()],
        output_type,
    )?;
    let mock = mount_sse_sequence(&server, responses).await;
...
async fn shell_output_preserves_fixture_json_without_serialization(
    ...
    let responses = shell_responses(
        call_id,
        vec![
            test_binary_path("sed"),
            "-n".to_string(),
            "p".to_string(),
            fixture_path_str.clone(),
        ],
        output_type,
    )?;
    let mock = mount_sse_sequence(&server, responses).await;
    ...
    let responses = vec![
        sse(vec![
            ...
            ev_local_shell_call(
                call_id,
                "completed",
                vec![test_binary_path("echo"), "local shell".to_string()],
            ),
            ev_completed("resp-1"),
        ]),
        sse(vec![
